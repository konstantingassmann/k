...
import { rect, createShader, createProgram, getAttribs, getUniforms, drawImage, createTexture, createTexCoordBuffer } from '../utils'
...
const uniforms = getUniforms(gl, program, ['matrix', 'image', 'letter', 'velocity'])

const imageTexCoordBuffer = createTexCoordBuffer(gl, attributes.texCoord, 8, 8)
const letterTexCoordBuffer = createTexCoordBuffer(gl, attributes.texCoord)
...
const draw = () => {
  ...
  cursorCtx.arc(mouseX, mouseY, 10, 0, Math.PI * 2)
  cursorCtx.stroke()
  ...
  drawImage({
    ...
    texcoordBuffer: imageTexCoordBuffer,
  ...
  drawImage({
    ...
    w: texWidth,
    h: texHeight,
    positionBuffer: texbuffer,
    texcoordBuffer: letterTexCoordBuffer,
    matrixLocation: uniforms.matrix,
    textureLocation: uniforms.image,
    texLocation: attributes.texCoord,
    positionLocation: attributes.position
